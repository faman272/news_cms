...
const Authors = ({ authors: initAuthors }) => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`/dashboard/authors/${deleteId}`);
  ...
  return (
    <>
      <Main>
        <h3 className="text-3xl font-medium text-gray-700">Data Authors</h3>
        {isDeleteModalOpen && (
        ...
        <div className="mt-8">
          ...
          <div class="mt-6">
            ...
            <div class="mt-3 flex flex-col sm:flex-row">
              ...
              <div>
                <a href='/dashboard/authors/create' class="flex items-center gap-2 px-6 py-2 bg-green-600 rounded-md text-white font-medium tracking-wide hover:bg-green-500 ml-3">
                ...
                <tbody>
                  {authors.map((author, index) => (
                    <tr key={author.id}>
                      ...
                      <td className="px-5 py-5 text-lg font-medium bg-white border-b border-gray-200">{author.name}</td>
                      <td className="px-5 py-5 text-lg bg-white border-b border-gray-200">{author.email}</td>
                      <td className="px-5 py-5 text-lg bg-white border-b border-gray-200">
                        <div className="inline-flex mt-2 xs:mt-0">
                          <a href={`/dashboard/authors/${author.id}`} className="px-4 py-2 font-semibold rounded-l bg-slate-200 hover:bg-slate-300 text-primary">
                            <FaRegEdit size={18} />
                          </a>
                          <button onClick={() => handleDeleteModal(author.id)} className="px-4 py-2 font-semibold text-red-600 rounded-r bg-slate-200 hover:bg-slate-300">
